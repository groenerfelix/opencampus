...
#summary(svm_model)
#tune_data <- tune(svm, Umsatz ~ Warengruppe + Wochentag + KielerWoche + Feiertag +  Temperatur + Windgeschwindigkeit, data=training_data, ranges = list(epsilon = seq(0, 1, 0.1), cost = 2^(1:2))) #original: 0.2, 1, 0.1, 2^(2:3)

tune_data <- tune(svm, Umsatz ~ Warengruppe * Wochentag + KielerWoche + Feiertag +  Warengruppe * Temperatur, data=training_data, ranges = list(epsilon = seq(0.2, 0.3, 0.02), cost = 2^(2:2))) #original: 0, 0.3, 0.05, 2^(1:2)
#0.1933 mit cost: 4, epsislon: 0.24
#ERGEBNISSE
#print(tune_data)
...
#TODO: für jedes Datum noch die sechs Warengruppen hinzufügen
neues_datum <- expand(neuer_zeitraum, Datum, Warengruppe = (1:6))
#neues_datum <- neuer_zeitraum %>% mutate(Warengruppe)
#neues_datum <- append(neues_datum, mutate(neuer_zeitraum, Warengruppe = 2))
