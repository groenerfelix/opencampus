...
# Confidence Interval
monat<-ggplot(sum_monat) +
  geom_bar( aes(x=Monat, y=mean), stat="identity", fill="darkgrey", alpha=0.5) +
  geom_errorbar( aes(x=Monat, ymin=mean-ic, ymax=mean+ic), width=0.4, colour="black", alpha=0.9, size=1.5) +
  ggtitle("Balkendiagramm für die Variable 'Monat' ")+
  xlab("Monate") + 
  ylab("Umsatz")
ggsave("sum_monat.png")

# Balkendiagram: gefühlte Temperatur
sum_temp <- umsatzdaten %>%
  group_by(gefuehlteTemp) %>%
  summarise(
    n=n(),
    mean=mean(Umsatz),
    sd=sd(Umsatz)
...
# Confidence Interval
temp<-ggplot(sum_temp) +
  geom_bar( aes(x=gefuehlteTemp, y=mean), stat="identity", fill="darkgrey", alpha=0.5) +
  geom_errorbar( aes(x=gefuehlteTemp, ymin=mean-ic, ymax=mean+ic), width=0.4, colour="black", alpha=0.9, size=1.5) +
