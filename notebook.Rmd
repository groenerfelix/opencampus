...
---
To-Do:
- Pattis und Finns Daten sowie weitere Feiertage einbinden.
...
#reg_data <- filter(umsatzdaten, Warengruppe == 1) %>% 
  reg_data <- select(umsatzdaten, Datum, Warengruppe, Umsatz, Wochentag, KielerWoche, Feiertag, Bewoelkung, Temperatur, Windgeschwindigkeit)
...
tune_data <- tune(svm, Umsatz ~ Warengruppe + Wochentag + KielerWoche + Feiertag +  Bewoelkung + Temperatur + Windgeschwindigkeit, data=training_data, ranges = list(epsilon = seq(0.2, 1, 0.1), cost = 2^(2:3))) 
